...
def tokenize_formula(formula: str):
    return formula.strip().split()


def tex2tree(formula: str) -> Node:
    tokens = [Symbol(i, token) for i, token in enumerate(tokenize_formula(formula))]
    return latex2Tree(tokens)


def tree_to_lines(node: Node, prefix: str = "", relation: str = "root"):
    label = getattr(node.x, "token", str(node.x))
    lines = [f"{prefix}[{relation}] {label}"]
    for child, child_relation in zip(node.childs, node.relations):
        if getattr(child, "x", None) == "<eol>":
            continue
        lines.extend(tree_to_lines(child, prefix + "  ", child_relation))
    return lines


if __name__ == "__main__":
    logger = get_logger("sandbox_latex2gtd")
    sample = "x ^ { 2 } + y _ { i }"
    parsed = tex2tree(sample)
    for line in tree_to_lines(parsed):
        logger.info("%s", line)
...
if __name__ == "__main__":
    ...
    tree = latex2Tree(latex_ls)
    gtd = node2list(tree)
    logger.info("=== sandbox_latex2gtd.py started ===")
    logger.info("original gtd:")
    for g in gtd:
        g = [str(item) for item in g]
        logger.info("%s", "\t\t".join(g))
    logger.info("%s", to_struct(latex.split()))
    logger.info("%s", tree_complexity(r"\sum _ { 1 } \frac { x ^ { 2 } + 1 } { y }"))
    logger.info("=== sandbox_latex2gtd.py finished ===")
